function buttonClicked() { // same function for every other js files, except brand is declared beforehand
    fetch(`https://makeup-api.herokuapp.com/api/v1/products.json?brand=benefit`)
        .then((response) => response.json())
        .then((data) => {
            var productsContainer = document.getElementById("products-container");
            productsContainer.innerHTML = ''; // to clear previous results 
            if (data.length > 0) {
                data.forEach((result) => {
                    ...
                    var lineBreak = document.createElement("br"); // for spaces in between, br
